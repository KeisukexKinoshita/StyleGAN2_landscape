...
if __name__ == "__main__":
    ...
    model.load(9)
    model.GAN.G.summary()
    model.GAN.steps=91000

    #for layer in model.GAN.generator.self.G.layers:
    for layer in model.GAN.G.layers:
        layer.trainable = False
    model.GAN.G.layers[-17].trainable= True 
    model.GAN.G.layers[-18].trainable= True 

    for layer in model.GAN.S.layers:
        layer.trainable = False
    model.GAN.S.layers[-1].trainable= True 
    model.GAN.S.layers[-2].trainable= True 

    for layer in model.GAN.D.layers:
        layer.trainable = False
    model.GAN.D.layers[-1].trainable= True 
    model.GAN.D.layers[-4].trainable= True 
    model.GAN.D.layers[-5].trainable= True 
    ...
    with tf.name_scope('summary'):
        tf.summary.scalar('d_loss', dl)
        tf.summary.scalar('g_loss', gl)
